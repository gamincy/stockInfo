...
Each pick also shows a short news note (where one was found) plus a
"More news" link to a live, continuously-updating source for that
ticker. Unlike the price data, news isn't regenerated by a script — it
requires a web search, so it's a snapshot from whenever it was last
compiled (see `data/PSE/news.json`'s `generated`/`note` fields). Ask
Claude to refresh it periodically for current picks.
